/*------------------------------------------
    JASON API BY Programming Hero Starts
--------------------------------------------*/
const loadProducts = () => {
...
/*------------------------------------------
      JASON API BY Programming Hero End
--------------------------------------------*/
/*------------------------------------------
  Showing All Products in User Interface.
--------------------------------------------*/
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    div.classList.add("col", "card-border");
    div.innerHTML = `<div class="card h-100"> <div class="card-image">
  <img src="${image}" class="mb-2 product-image" alt="..."></div>
  ...
  <div class="card-body">
    <button onclick="addToCart(${product.id},${product.price})" id="addToCart-btn" class="buy-now btn btn-success">add to cart</button>
      <button type="button" id="details-btn" class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#staticBackdrop">Details</button>
  </div>
</div>`;
...
const getInputValue = (id) => {
  ...
  // parseINT To parseFloat Method for Float Input
  const converted = parseFloat(element);
  return converted;
};
/*------------------------------------------
          Main Value Update Function
--------------------------------------------*/
const updatePrice = (id, value) => {
  const convertedOldPrice = getInputValue(id);
  const convertPrice = parseFloat(value);
  const total = convertedOldPrice + convertPrice;
  // Remove MathRound Method For Showing Fixed Float Value.
...
loadProducts();
/*----------------------------------------------------
            Finish JavaScript Bug Fixing
-------------------------------------------------------*/
/*----------------------------------------------------
                      Thank You
              Programming Hero Examiner
                   Have a Nice Day
-------------------------------------------------------*/